...
setTimeout(() => { console.log("Waiting 15 seconds!"); }, delayInMilliseconds);
...
client.executeBulkQueries().then(result => {
  console.log("2 - Executing Bulk Queries.  Results are \n");
  console.log(result);
  for (let i=0; i < result.length; i++) {
    if (result[i].result.value > 80) {
      console.log(result[i].indicator + "pattern matches with value of " + result[i].result.value);
...
// Init bulk queries. This resets all previously added queries. Max of 20
console.log("3- Clear Bulk API and Load up with 20");
client.initBulkQueries();
...
// execture bulk queries
client.executeBulkQueries().then(result => {
  console.log("3- Executing Bulk Queries.  Results are \n");
  console.log(result);
